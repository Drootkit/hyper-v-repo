...
if URL contains "mp.weixin.qq.com":
    → python3 G:\MCP\qiaomu-markdown-proxy/fetch_weixin.py "URL"
    → Done

if URL contains "feishu.cn/docx/" or "feishu.cn/wiki/" or "larksuite.com/docx/":
    → python3 G:\MCP\qiaomu-markdown-proxy/fetch_feishu.py "URL"
    → Done
...
if URL ends with ".pdf" or is local PDF path:
    ...
    if local path:
        → bash G:\MCP\qiaomu-markdown-proxy/extract_pdf.sh "PATH"
    → Done

else:
    → bash G:\MCP\qiaomu-markdown-proxy/fetch.sh "URL"
    → Done
```
...
```bash
bash G:\MCP\qiaomu-markdown-proxy/fetch.sh "https://example.com/article"
```

### X/Twitter Post

```bash
bash G:\MCP\qiaomu-markdown-proxy/fetch.sh "https://x.com/username/status/1234567890"
```

### WeChat Article

```bash
python3 G:\MCP\qiaomu-markdown-proxy/fetch_weixin.py "https://mp.weixin.qq.com/s/abc123"
```

### Feishu Document

```bash
python3 G:\MCP\qiaomu-markdown-proxy/fetch_feishu.py "https://xxx.feishu.cn/docx/xxxxxxxx"
```
...
```bash
bash G:\MCP\qiaomu-markdown-proxy/extract_pdf.sh "/path/to/paper.pdf"
```

### With Custom Proxy

```bash
bash G:\MCP\qiaomu-markdown-proxy/fetch.sh "https://example.com" "http://127.0.0.1:7890"
```
